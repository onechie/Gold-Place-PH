$(document).ready(function () {
          ...
          for (let i = 0; i < itemInfo.images.length; i++) {
            addOutput.append(
              "<div class='ratio ratio-1x1 bg-light shadow rounded-4 align-self-center' style='max-width: 300px; margin:10px;'>" +
                "<img class='inputImages rounded-4' src='../assets/images/items/" +
                itemInfo.id +
                "/" +
                itemInfo.images[i] +
                "'/>" +
  ...
  $("#imageInput").change(function () {
    ...
    while (i < this.files.length) {
      ...
      if ($.inArray(fileType, validImageTypes) < 0) {
        addMsg.empty();
        addMsg.append(
          "<span class='text-danger'>Please insert pictures only!</span>"
        );
        addOutput.empty();
        this.value = null;
      } else {
    ...
    while (switching) {
      ...
      for (i = 0; i < rows.length - 1; i++) {
        ...
        if (by == "id") {
          ...
          y = Number(
            $(rows[i + 1].getElementsByTagName("td")[2])
              .children("p")
              .children(".stocks")
              .text()
              .toLowerCase()
          ...
          y = Number(
            $(rows[i + 1].getElementsByTagName("td")[3])
              .children(".price")
              .text()
              .toLowerCase()
          );
      ...
      if (data && data != "null") {
        ...
        for (let i = 0; i < itemInfo.length; i++) {
          ...
          htmlData +=
            ...
            "   <td class='ps-4 py-4 id sort-by-id'>" +
            item.id +
            "</td>" +
            "    <td class='ps-4'>" +
            "        <div class='d-flex align-items-center'>" +
            "            <div class='position-relative rounded-5 bg-white shadow-sm me-2 flex-shrink-0' style='width:50px; height:50px'>" +
            "                <img class='position-absolute m-1 bg-secondary rounded-5' src='../assets/images/items/" +
            item.id +
            "/" +
            item.images[0] +
            "' alt='' style='width:42px; height:42px'>" +
            "           </div>" +
            "            <strong id='sort-by-name' class='text-break'>" +
            item.name +
            "</strong>" +
            "        </div>" +
            "    </td>" +
            "    <td class='ps-4 stocks-parent'>" +
            "        <p class='p-0 m-0'><span class='stocks'>" +
            item.stocks +
            "</span>/100</p>" +
            ...
            "        </div>" +
            "    </td>" +
            "    <td class='ps-4'><strong class='price text-break'>" +
            item.price +
            "</strong></td>" +
            "    <td class='ps-4'><strong class='sold'>" +
            item.sold +
            "</strong></td>" +
            "    <td class='ps-4 d'>" +
            "        <div class='d-flex '>" +
  ...
  function serverResponseTranslate(data) {
    ...
    if (data == "addSuccess") {
      addMsg.append(
        "<span class='text-success'>Item uploaded successfully!</span>"
      );
      addForm[0].reset();
      addOutput.empty();
    }
    if (data == "editSuccess") {
      addMsg.append(
        "<span class='text-success'>Item edited successfully!</span>"
      );
    }
    if (data == "addFailed") {
    ...
    if (data == "largeImage") {
      addMsg.append(
        "<span class='text-danger'>File size is too large! (maximum size 2mb)</span>"
      );
    }
    if (data == "formatImage") {
      addMsg.append(
    ...
    if (data == "deleteSuccess") {
      $("#confirmation #messageText").append(
        "<span class='text-success'>Item deleted successfully!</span>"
      );
    }
    if (data == "deleteFailed") {
      $("#confirmation #messageText").append(
        "<span class='text-danger'>Item deletion failed!</span>"
      );
    }

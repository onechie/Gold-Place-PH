$(document).ready(function () {
  ...
  const modalRatings = $("#item #item-ratings");
    ...
    $.post(
      ...
      function (data) {
        if (data && data != "null") {
          ...
          for (let i = 0; i < item.images.length; i++) {
            ...
            modalImage.append(
              "<div class='carousel-item " +
                setActive +
                " ratio ratio-1x1' style='max-width: 400px;'>" +
                "    <img src='./assets/images/items/" +
                item.id +
                "/" +
                item.images[i] +
                "' class='d-block rounded-4' alt='image' style='object-fit: cover;'>" +
                "</div>"
            );
          }
          ...
          modalRatings.text(item.ratings.length)

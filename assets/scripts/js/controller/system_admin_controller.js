$(document).ready(function () {
  ...
  const fnWarning = $("#add-user .fn-w");
  const lnWarning = $("#add-user .ln-w");
  const emWarning = $("#add-user .em-w");
  const phWarning = $("#add-user .ph-w");
  const pwWarning = $("#add-user .pw-w");
    ...
    $.post(
      ...
      function (data) {
        if (data == "ok") {
          ...
          */
          toastBody.text("User Created successfully!");
          toast.show();
          reset();
        } else {
        ...
        if (data && data != "null") {
          ...
          for (let i = 0; i < users.length; i++) {
            ...
            htmlData +=
              ...
              "</strong></td>" +
              "    <td class='ps-4'>" +
              "        <i class='editUser bi bi-pencil-square fs-5 text-success icon-btn' data-bs-toggle='modal' data-bs-target='#edit-user'></i>" +
              "        <i class='deleteUser bi bi-trash fs-5 text-danger icon-btn' data-bs-toggle='modal' data-bs-target='#confirm'></i>" +
              "    </td>" +
              "</tr>";
          }
